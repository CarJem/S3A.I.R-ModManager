...
{
    public partial class KeyBindingDialogOriginal : Form
    {
        ...
        private KeyBindingDialogOriginal Instance;

        public KeyBindingDialogOriginal()
        {
            InitializeComponent();
            Instance = this;
        ...
        private void UpdateRadioButtons(string currentBinding)
        {
            if (currentBinding.Contains("Button") || currentBinding.Contains("Axis") || currentBinding.Contains("Thumb") || currentBinding.Contains("POV"))
            {
                if (currentBinding.Contains("Button") && Extensions.IsDigitsOnly(currentBinding.Replace("Button", "")))
                {
                    int index = int.Parse(currentBinding.Replace("Button", ""));
                }
                else
                {
                    int index = 0;
                    string axisType = "";
                    if (currentBinding.Contains("Axis") && Extensions.IsDigitsOnly(currentBinding.Replace("Axis", "")))
                    {
                        index = int.Parse(currentBinding.Replace("Axis", ""));
                        axisType = "Axis";
                    }
                    else if (currentBinding.Contains("Thumb") && Extensions.IsDigitsOnly(currentBinding.Replace("Thumb", "")))
                    {
                        index = int.Parse(currentBinding.Replace("Thumb", ""));
                        axisType = "Thumb";
                    }
                    else if(currentBinding.Contains("POV") && Extensions.IsDigitsOnly(currentBinding.Replace("POV", "")))
                    {
                        index = int.Parse(currentBinding.Replace("POV", ""));
                        axisType = "Pov";
                    }
            ...
            else if (KeyBindings.FindIndex(collection => collection.SequenceEqual(currentBinding)) != -1)
            {

            }
            else if (currentBinding.Equals("NONE"))
            {
                ...
                else if (controllerInputTypeRadio2.Checked)
                {
                    ...
                    if (axisType == "Pov")
                    {
                        if (up)
                        {
                            ...
                            resultText.Tag = $"{axisType}{axisBase}";
                        }
                        else if (right)
                        {
                            resultText.Text = $"{axisType}{axisBase + 1}";
                            resultText.Tag = $"{axisType}{axisBase + 1}";
                        }
                        else if (down)
                        {
                            resultText.Text = $"{axisType}{axisBase + 2}";
                            resultText.Tag = $"{axisType}{axisBase + 2}";
                        }
                        else if (left)
                        {
                            resultText.Text = $"{axisType}{axisBase + 3}";
                            resultText.Tag = $"{axisType}{axisBase + 3}";
                        }
